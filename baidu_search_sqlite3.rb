...
db.execute("create table results(id integer PRIMARY KEY autoincrement, count integer, keyword varchar, number integer, title varchar, url varchar, created_at varchar)")
db.close
...
while $count < $total
  ...
  if $count.even?
    if $results.count.eql?(0)
      ...
      db = SQLite3::Database.open("list.db")
      $results.each do |result|
        db.execute "insert into results(count, keyword, number, title, url, created_at) values(?, ?, ?, ?, ?, ?)", result[:count], result[:keyword], result[:number], result[:title], result[:url], result[:created_at]
      end
      db.close
      $results.clear
    end
